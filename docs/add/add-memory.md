...
> 版本：v3.1 | 日期：2026-07-14 | 状态：draft | 插件名：`memory`
>
> **v3.1 变更**：即时偏好引擎从 v3.0 的"Router 自学习核心"降级为**辅助机制**，仅负责内容生成偏好。路由策略优化由新的 [Router 自学习引擎](add-router-learning.md) 负责。两者独立工作。

## 1. 背景

用户需要 AI 记住偏好、身份、习惯和重要信息，跨会话持久化。**即时偏好引擎**在用户声明偏好时实时写入，无需等待后台任务。下次对话自动注入偏好上下文。

v3.1 起，即时偏好引擎的职责明确为**内容生成偏好**（如写作格式、语言、风格），与 [Router 自学习引擎](add-router-learning.md) 负责的**路由策略优化**（用户输入 -> 哪个 Subagent）独立工作。两者共同构成 Prometheus 的"自我进化"能力。
...
### 3.2 即时偏好引擎（辅助机制，v3.1 降级）

v3.1 重大变更：即时偏好引擎从 v3.0 的"Router 自学习核心"降级为**辅助机制**。

**职责边界**：

| 机制 | 职责 | 影响范围 | 详细设计 |
|------|------|---------|---------|
| Router 自学习引擎 | 路由策略优化 | 用户输入 -> 哪个 Subagent | [add-router-learning.md](add-router-learning.md) |
| 即时偏好引擎（本节） | 内容生成偏好 | Subagent 生成内容时用什么格式/风格 | 本节 |

即时偏好引擎仍保留其在 v3.0 的核心价值：用户声明偏好时实时写入 memory，不依赖后台 cron 任务，演示中即时展示"越用越懂你"的内容生成偏好效果。
