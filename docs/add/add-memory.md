...
> 版本：v1.0 | 日期：2026-07-12 | 状态：draft | 插件名：`memory`

## 1. 背景

用户需要 AI 记住偏好、身份、习惯和重要信息，跨会话持久化。后台反思引擎提取行为模式，实现主动推送和个性化服务。这是 Prometheus "自我进化" 核心差异化所在。
...
| 触发方式 | 频率 | 实现 |
|---------|------|------|
| 定时任务 | 每日凌晨 2:00 | cron + Python 脚本 |
| 阈值触发 | 每 50 次交互 | interaction_log 计数 → 增量反思 |
| 手动触发 | 用户主动 | "帮我总结一下最近" → chat_light + memory_recall |
| 事件触发 | rag_ingest / new session | 新文档摄入 → 实体抽取 + 图谱关联 |
...
- 高频主题：HICOOL 智能体赛道（出现 8 次）→ 已创建知识图谱节点
```
...
- [ ] MEM-012 实现 `get_recent_interactions()` — 反思引擎加载最近日志

### 反思引擎

- [ ] MEM-013 实现 `reflection_engine.py` — `run_reflection_cycle()` 主循环
- [ ] MEM-014 实现 `extract_reminder_candidates()` — 检测重复时间行为
- [ ] MEM-015 实现 `extract_preference_changes()` — 检测偏好声明和变化
- [ ] MEM-016 实现 `extract_hot_topics()` — 聚类高频主题（>= 3 次）
- [ ] MEM-017 实现反思摘要格式化（Markdown）
- [ ] MEM-018 实现 `memory_injector.py` — 请求前自动注入记忆上下文到 system prompt

### 定时任务

- [ ] MEM-019 编写 `reflection_loop.sh` — 反思循环入口
- [ ] MEM-020 设置 cron 定时任务（每日凌晨 2:00 + 每 50 次交互阈值触发）
...
- [ ] MEM-028 单元测试：反思引擎提取偏好变化
- [ ] MEM-029 单元测试：反思引擎提取高频主题
