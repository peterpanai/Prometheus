...
本设计针对当前要开发的 5 个 Subagent（RAG / 记忆与偏好 / 写作润色翻译 / 日程与任务 / 闲聊陪伴）及 2 个赛题加分项模块（Router 自学习引擎 / Subagent 市场），分别调研了 Hermes（`~/ws/hermes-agent`）、OpenClaw（`~/ws/openclaw`）、Codex（`~/ws/codex`）、MTClaw Function Router 以及 VS Code / npm 等成熟系统中对应领域的实现，提炼可借鉴点。各 Subagent / 模块的详细调研见 `docs/add/` 下对应文档的 §2 调研章节。
...
**v3.0 关键改进**：工具数从 24 降到 16，接近 LLM function calling 准确率的最佳范围（研究表明 <15 工具时准确率最高 [推测]，16 个略超但通过 5 个 Subagent 清晰划分保持路由精度）。
